    	...
    	try {
			...
			if(sequenceRS.next()){
				...
				Statement stmtDistance = conn.createStatement();
	    			ResultSet distanceQuery = stmtDistance.executeQuery("SELECT name, ST_Distance(latlng,ST_GeographyFromText('"+ n.getLatLng() +"')) AS distan FROM BusStop WHERE id='"+nextStop+"'");
